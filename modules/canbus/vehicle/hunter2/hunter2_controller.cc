...
ErrorCode Hunter2Controller::Init(
  ...
  if (brake_control_command_131_ == nullptr) {
    AERROR << "Brakecontrolcommand131 does not exist in the "
              "Hunter2MessageManager!";
    return ErrorCode::CANBUS_ERROR;
  }
  ...
  can_sender_->AddMessage(Brakecontrolcommand131::ID,
                          brake_control_command_131_, false);
  can_sender_->AddMessage(Controlmodesetting421::ID, control_mode_setting_421_,
                          false);
  can_sender_->AddMessage(Motioncontrolinstruction111::ID,
                          motion_control_instruction_111_, false);
...
Chassis Hunter2Controller::chassis() {
  ...
  if (chassis_detail.hunter2().has_motion_control_feedback_221() &&
    ...
    chassis_.set_speed_mps(static_cast<float>(
        chassis_detail.hunter2().motion_control_feedback_221().speed()));
  } else {
  ...
  if (chassis_detail.hunter2().has_motion_control_feedback_221() &&
    ...
    chassis_.set_steering_percentage(static_cast<float>(
        ...
        vehicle_params_.max_steer_angle() * 100.0));
  } else {
...
bool Hunter2Controller::VerifyID() { return true; }
