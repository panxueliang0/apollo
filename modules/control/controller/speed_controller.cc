...
SpeedController::SpeedController()
  ...
  if (FLAGS_enable_csv_debug) {
    ...
    strftime(name_buffer, 80, "/apollo/data/log/speed_log__%F_%H%M%S.csv",
             &time_tm);
    speed_log_file_ = fopen(name_buffer, "w");
    AINFO << "debug name" << name_buffer;
...
Status SpeedController::Init(std::shared_ptr<DependencyInjector> injector,
  ...
  if (control_conf_ == nullptr) {
    ...
    return Status(ErrorCode::CONTROL_INIT_ERROR,
                  "Failed to load SpeedController conf");
  }
  injector_ = injector;
  ...
  bool enable_leadlag =
      speed_controller_conf.enable_reverse_leadlag_compensation();

  station_pid_controller_.Init(speed_controller_conf.station_pid_conf());

  if (enable_leadlag) {
    station_leadlag_controller_.Init(
        speed_controller_conf.reverse_station_leadlag_conf(), ts);
    speed_leadlag_controller_.Init(
        speed_controller_conf.reverse_speed_leadlag_conf(), ts);
  }

  vehicle_param_.CopyFrom(
      common::VehicleConfigHelper::Instance()->GetConfig().vehicle_param());

  SetDigitalFilterPitchAngle(speed_controller_conf);
...
void SpeedController::SetDigitalFilterPitchAngle(
    const SpeedControllerConf &speed_controller_conf) {
  double cutoff_freq =
      speed_controller_conf.pitch_angle_filter_conf().cutoff_freq();
  double ts = speed_controller_conf.ts();
  SetDigitalFilter(ts, cutoff_freq, &digital_filter_pitch_angle_);
}

void SpeedController::LoadControlCalibrationTable(
    const SpeedControllerConf &speed_controller_conf) {
  const auto &control_table = speed_controller_conf.calibration_table();
...
Status SpeedController::ComputeControlCommand(
  ...
  double ts = speed_controller_conf.ts();
  double preview_time = speed_controller_conf.preview_window() * ts;
  bool enable_leadlag =
      speed_controller_conf.enable_reverse_leadlag_compensation();
  ...
  double station_error_limit = speed_controller_conf.station_error_limit();
  ...
  if (trajectory_message_->gear() == canbus::Chassis::GEAR_REVERSE) {
    station_pid_controller_.SetPID(
        speed_controller_conf.reverse_station_pid_conf());
    if (enable_leadlag) {
      station_leadlag_controller_.SetLeadlag(
          speed_controller_conf.reverse_station_leadlag_conf());
      speed_leadlag_controller_.SetLeadlag(
          speed_controller_conf.reverse_speed_leadlag_conf());
    }
  } else if (injector_->vehicle_state()->linear_velocity() <=
             speed_controller_conf.switch_speed()) {
    station_pid_controller_.SetPID(speed_controller_conf.station_pid_conf());
  } else {
    station_pid_controller_.SetPID(speed_controller_conf.station_pid_conf());
  }
  ...
  double speed_controller_input_limit =
      speed_controller_conf.speed_controller_input_limit();
  double speed_controller_input_limited = 0.0;
  if (FLAGS_enable_speed_station_preview) {
    speed_controller_input = speed_offset + debug->preview_speed_reference();
  } else {
    speed_controller_input = speed_offset + debug->speed_reference();
  }
  speed_controller_input_limited =
  ...
  debug->set_throttle_cmd(speed_controller_input_limited);
  ...
  if (FLAGS_enable_csv_debug && speed_log_file_ != nullptr) {
    fprintf(speed_log_file_,
            ...
            station_error_limited, debug->preview_station_error(), speed_offset,
            debug->speed_reference(), speed_controller_input_limited,
            debug->speed_error(), debug->preview_speed_reference(),
  ...
  cmd->set_throttle(speed_controller_input_limited);
...
void SpeedController::SetDigitalFilter(double ts, double cutoff_freq,
...
void SpeedController::GetPathRemain(SimpleLongitudinalDebug *debug) {
  int stop_index = 0;
