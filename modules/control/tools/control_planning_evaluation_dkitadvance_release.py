...
class ControlInfo(object):
    ...
    def callback_control(self, entity):
        ...
        #print"throttle is\n",self.throttle
        #self.throttle.append(entity.debug.simple_lon_debug.throttle)
        #self.control_cmd.throttle(entity.debug.control_cmd.throttle)
        #print"break is\n",self.break
        #self.break.append(entity.debug.simple_lon_debug.break)
    ...
    def control_planning_dif(self, t=False):
        """
        ...
        if not self.print_traj:
            ...
            self.ax[0, 0].plot(
                self.controltime, self.current_station, linewidth=1.0, label='current_station')
            ...
            legend=self.ax[0, 0].legend(fontsize='small', loc='upper left')
        ...
        if not self.print_traj:
            self.ax[0, 1].plot(
                self.controltime, self.lateral_error, linewidth=1.0, label='lateral_error')
                #self.controltime, self.lateral_error)
            # legend=self.ax[0, 1].legend(fontsize='small',loc='upper left')
            legend=self.ax[0, 1].legend(fontsize='small')
            frame=legend.get_frame()
            frame.set_alpha(1)
            frame.set_facecolor('none')
        ...
        # speed_error
        self.speed_reference = self.speed_reference[del_head_error:del_end_error]
        ...
        if not self.print_traj:
            self.ax[1, 0].plot(
                self.controltime, self.speed_reference, linewidth=1.0, label='speed_reference')
                #self.controltime, self.speed_reference)
            self.ax[1, 0].plot(
                self.controltime, self.current_speed, linewidth=1.0, label='current_speed')
                #self.controltime, self.current_speed)
            self.ax[1, 0].plot(
                self.controltime, self.speed_error, linewidth=1.0, label='speed_error')
                #self.controltime, self.speed_error)
            legend=self.ax[1, 0].legend(fontsize='small', loc='upper left')
            frame=legend.get_frame()
            frame.set_alpha(1)
            frame.set_facecolor('none')
        ...
        if not self.print_traj:
            self.ax[1, 1].plot(
                self.controltime, self.ref_heading, linewidth=1.0, label='ref_heading')
                #self.controltime, self.ref_heading)
            self.ax[1, 1].plot(
                self.controltime, self.heading, linewidth=1.0, label='heading')
                #self.controltime, self.heading)
            self.ax[1, 1].plot(
                self.controltime, self.heading_error, linewidth=1.0, label='heading_error')
                #self.controltime, self.heading_error)
            legend=self.ax[1, 1].legend(fontsize='small')#, mode='expend', shadow=True, bbox_to_anchor=(1.05,1), loc = 'upper left', borderaxespad=0.
            frame=legend.get_frame()
            frame.set_alpha(1)
            frame.set_facecolor('none')
        ...
        if self.print_traj:
            ...
            self.ax.plot(
                self.planningx, self.planningy, linewidth=2.0, linestyle='--',label='planning path')
            self.ax.plot(
                self.carx, self.cary, linewidth=1.0, label='pose')
            self.ax.plot(
                self.pathx, self.pathy, linewidth=1.0, label='planning path remain')
            legend=self.ax.legend(fontsize='small')
            frame=legend.get_frame()
            frame.set_alpha(1)
            frame.set_facecolor('none')
            self.ax.grid(True)
        ...
        sheet.write(file_num, 3, format(average_station_error, '.3f'), book_format)
        station_error_grade = self.get_station_error_grade(max_station_error)
        station_error_stype = self.get_grade_style(station_error_grade)
        station_error_stype.set_bold()
        sheet.write(file_num, 4, station_error_grade, station_error_stype)
        ...
        sheet.write(file_num, 7, format(average_lateral_error, '.3f'), book_format)
        lateral_error_grade = self.get_lateral_error_grade(max_lateral_error)
        lateral_error_stype = self.get_grade_style(lateral_error_grade)
        lateral_error_stype.set_bold()
        sheet.write(file_num, 8, lateral_error_grade, lateral_error_stype)
        ...
        sheet.write(file_num, 11, format(average_speed_error, '.3f'), book_format)
        speed_error_grade = self.get_speed_error_grade(max_speed_error)
        speed_error_stype = self.get_grade_style(speed_error_grade)
        speed_error_stype.set_bold()
        sheet.write(file_num, 12, speed_error_grade, speed_error_stype)
        ...
        sheet.write(file_num, 15, format(average_heading_error, '.3f'), book_format)
        heading_error_grade = self.get_heading_error_grade(max_heading_error)
        heading_error_stype = self.get_grade_style(heading_error_grade)
        heading_error_stype.set_bold()
        sheet.write(file_num, 16, heading_error_grade, heading_error_stype)

        average_score = format((station_error_grade + lateral_error_grade + speed_error_grade + heading_error_grade) / 4.0, '.2f')
        ...
        # debug for carxy
        sheet.write_column('AB2', self.carx)
        sheet.write_column('AC2', self.cary)
        sheet.write_column('AD2', self.cartime)
        
        sheet.write_column('AF2', self.planningx)
        sheet.write_column('AG2', self.planningy)
        
        if args.path and not self.print_traj:
            average_score = format((station_error_grade + lateral_error_grade + speed_error_grade + heading_error_grade) / 4.0, '.2f')
            global score_num
            score_num = float(score_num) + float(average_score)
            #book_format = book.add_format({'align':'center','text_wrap': True})
            book_format17 = book.add_format({'font_color': 'black', 'align': 'center'})
            book_format17.set_bold()
            case_sheet.write(record_num, 0, file)
            if flag:
                i = 1
                #print"irows is:", irows                         
                for i in range(irows):
                    if file.lower() == case_table.cell_value(i, 0).lower() :                    
                        #print "i is:", i
                        #print"file_name is:", file
                        value = case_table.cell(i,1).value.encode('UTF-8')                
                        #print"cell_value is:", value                 
                        case_sheet.write(record_num, 1, value)
                        value = case_table.cell(i,2).value               
                        #print"cell_value is:", value
                        if case_table.cell(i,2).ctype == 2 and value % 1 == 0 :
                            case_sheet.write(record_num, 2, int(value))
                        else:
                            case_sheet.write(record_num, 2, value)
                        value = case_table.cell(i,3).value               
                        #print"cell_value is:", value
                        if case_table.cell(i,3).ctype == 2 and value % 1 == 0 :
                            case_sheet.write(record_num, 3, int(value))
                        else:
                            case_sheet.write(record_num, 3, value)
                        value = case_table.cell(i,4).value.encode('UTF-8')                
                        #print"cell_value is:", value 
                        case_sheet.write(record_num, 4, value)
                        value = case_table.cell(i,5).value.encode('UTF-8')                
                        #print"cell_value is:", value
                        case_sheet.write(record_num, 18, value)                
            case_sheet.write(record_num, 1, vehicel_behavior, book_format)
            case_sheet.write(record_num, 2, format(max_speed, '.3f'), book_format)
            case_sheet.write(record_num, 3, int(max_station), book_format)
            case_sheet.write(record_num, 5, format(max_station_error, '.3f'), book_format)
            case_sheet.write(record_num, 6, format(average_station_error, '.3f'), book_format)
            case_sheet.write(record_num, 7, station_error_grade, station_error_stype)
            case_sheet.write(record_num, 8, format(max_lateral_error, '.3f'), book_format)
            case_sheet.write(record_num, 9, format(average_lateral_error, '.3f'), book_format)
            case_sheet.write(record_num, 10, lateral_error_grade, lateral_error_stype)
            case_sheet.write(record_num, 11, format(max_speed_error, '.3f'), book_format)
            case_sheet.write(record_num, 12, format(average_speed_error, '.3f'), book_format)
            case_sheet.write(record_num, 13, speed_error_grade, speed_error_stype)
            case_sheet.write(record_num, 14, format(max_heading_error, '.3f'), book_format)
            case_sheet.write(record_num, 15, format(average_heading_error, '.3f'), book_format)
            case_sheet.write(record_num, 16, heading_error_grade, heading_error_stype)
            case_sheet.write(record_num, 17, average_score, book_format17)
    ...
    def get_grade_style(self, value):
        style = ""
        style_dict = {1 : book.add_format({'align':'center','font_color': 'red'}),
                      2 : book.add_format({'align':'center','font_color': 'brown'}),
                      3 : book.add_format({'align':'center','font_color': 'orange'}),
                      4 : book.add_format({'align':'center','font_color': 'blue'}),
                      5 : book.add_format({'align':'center','font_color': 'dark_green'})}
        if value == 5:
            style = style_dict[5]
        elif value == 4:
            style = style_dict[4]
    ...
    def read_bag(self, bag_file):
        ...
        bag = record.RecordReader(file_path)
        print ("Begin reading the file: ", file_path)
        #for topic, msg, t in bag.read_messages(topics=[        
                #'/apollo/control', '/apollo/planning', '/apollo/canbus/chassis']):
        ...
        #print"len of speed_reference is %d \n"%(len(controlinfo.speed_reference)), controlinfo.speed_reference
        #print"current_speed is \n", controlinfo.current_speed
        #print"speed_error is \n", controlinfo.speed_error
        #print"ref_heading is \n", controlinfo.ref_heading
        #print"current_heading is \n", controlinfo.heading
        #print"heading_error is \n", controlinfo.heading_error
        #print"station_reference is \n", controlinfo.station_reference
        #print"current station is \n", controlinfo.current_station
        #print"station error ir \n", controlinfo.station_error
        print( "Done reading the file: ", file_path)
        #bag.close()
        #cyber.shutdown()

    def add_excel_sheet(self):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=
        "Process and analyze control and planning data. usage:\
    ...
    if args.path:
        ...
        excel_name = dir_path + 'evaluation_result.xlsx'
        # print "excel name is", excel_name
        case_file_name = dir_path + 'case.xlsx'
        flag = os.path.exists(case_file_name)
        # print "case.xlsx exists: ",flag
        if flag:         
            case_book = xlrd.open_workbook(case_file_name)
            case_table = case_book.sheet_by_index(0)
            irows = case_table.nrows
        #book = xlsxwriter.Workbook(dir_path + '/' + excel_name)
        ...
        if os.path.isdir(dir_path):
            ...
            for file in dirs:
                ...
                if is_record_file(file):
                    ...
                    controlinfo.read_bag(dir_path + file)
                    sheet = controlinfo.add_excel_sheet()
                    ...
                    controlinfo = ControlInfo(axarr)
                    controlinfo.read_bag(dir_path + file)
                    controlinfo.control_planning_dif(trajectory_print)
                    fig.canvas.mpl_connect('key_press_event', controlinfo.press)
        ...
        case_sheet.write(record_num, 17, '总分：' + bytes(score_num))
        record_num = record_num + 1
        string_value = "总体主观评价"
        case_sheet.merge_range(record_num, 0, record_num, 1, string_value)
        if flag:
            for i in range(irows):
                if string_value in case_table.cell(i, 0).value :
                    case_sheet.merge_range(record_num, 2, record_num, 16, case_table.cell_value(i,1))
        case_sheet.write(record_num, 17, '平均：' + bytes(format(score_num / (record_num - 3), '.3f')))
        ...
        fig.canvas.mpl_connect('key_press_event', controlinfo.press)
        plt.show()
        #output_results.save('./evaluation_result.xls')
    cyber.shutdown()
