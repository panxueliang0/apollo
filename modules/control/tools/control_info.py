...
class ControlInfo(object):
    ...
    def __init__(self,axarr):
        ...
        self.acc_open = []
        self.acc_close = []  #speed_PID的输出
        self.slope_offset_compensation = []  #s斜坡加速度
        self.acceleration_lookup = []   #查表用的加速度值
        self.speed_lookup = []  #查表用的速度值
        self.calibration_value = []  #查表所得油门指令
        ...
        self.numpoints_sum=[]
        self.planning_sum=[]
        self.show_planning_flag=True   #是否展示planning信息的参数
        self. count=1
        self.axarr = axarr
    ...
    def __callback_planning(self, entity):
        ...
        if numpoints == 0:
            ...
            if self.show_planning_flag:
                self.axarr.plot(
                        self.planning_pathx, self.planning_pathy, linewidth=2.0, linestyle='--')
                plt.draw()
    ...
    def __callback_control(self, entity):
        ...
        self.calibration_value.append(entity.debug.simple_lon_debug.calibration_value)
        ...
        self.pid_saturation_status.append(entity.debug.simple_lon_debug.pid_saturation_status)
        ...
        self.station_reference.append(entity.debug.simple_lon_debug.station_reference)
        self.current_station.append(entity.debug.simple_lon_debug.current_station)
        self.station_error.append(entity.debug.simple_lon_debug.station_error)
        self.station_error_limited.append(entity.debug.simple_lon_debug.station_error_limited)
        ...
        self.speed_controller_input_limited.append(entity.debug.simple_lon_debug.speed_controller_input_limited)
        
        self.speed_reference.append(entity.debug.simple_lon_debug.speed_reference)
        ...
        self.heading_error_feedback.append(entity.debug.simple_lat_debug.heading_error_feedback)
        self.lateral_error.append(entity.debug.simple_lat_debug.lateral_error)
        self.lateral_error_feedback.append(entity.debug.simple_lat_debug.lateral_error_feedback)
        ...
        self.steer_angle_feedback.append(entity.debug.simple_lat_debug.steer_angle_feedback)
        self.steer_angle_feedforward.append(entity.debug.simple_lat_debug.steer_angle_feedforward)
        self.steer_angle_feedback_augment.append(entity.debug.simple_lat_debug.steer_angle_feedback_augment)
        self.steer_angle_lateral_contribution.append(entity.debug.simple_lat_debug.steer_angle_lateral_contribution)
    ...
    def plot_station_speed(self):
        ...
        ax[0].plot(
            self.controltime, self.station_reference, marker = '*',markersize=2,linewidth=1.0, label='station_reference')
        ax[0].plot(
            self.controltime, self.current_station, marker = '*',markersize=2,linewidth=1.0, label='current_station')
        # self.ax[0, 1].plot(
        #     self.controltime, self.station_error, linewidth=1.0, label='station_error')      
        ax[0].plot(
            self.controltime, self.station_error_limited, marker = '*',markersize=2,linewidth=1.0, label='station_error_limited')      
        ax[0].plot(
        ...
        ax[1].plot(
                self.controltime, self.speed_error, marker = '*',markersize=2,linewidth=1.0, label='speed_error')
        ax[1].plot(
        ...
        ax[1].plot(
                self.controltime, self.speed_offset, marker = '*',markersize=2,linewidth=1.0, label='speed_offset')
        ax[1].plot(self.controltime, self.current_speed,marker = '*',markersize=2, label='current_speed')
        ax[1].plot(
    ...
    def plot_PID(self):
        ...
        ax[0].legend(fontsize='medium')
        ax[0].grid(True)
        ax[0].set_title('Station PID')
        ax[0].set_xlabel('Time-s')
        ax[0].set_ylabel('Station-PID')
        ...
        ax[1].legend(fontsize='medium')
        ax[1].grid(True)
        ax[1].set_title('Speed PID')
        ax[1].set_xlabel('Time-s')
        ax[1].set_ylabel('Speed-PID')
    ...
    def plot_matrix_kq(self):
        fig, ax = plt.subplots(2, 1)
        ax[0].get_shared_x_axes().join(ax[0], ax[1])

        ax[0].plot(self.controltime, self.matrix_k_0, marker = '*',markersize=2,linewidth=1.0, label='matrix_k_0')
        ax[0].plot(self.controltime, self.matrix_k_1, marker = '*',markersize=2,linewidth=1.0, label='matrix_k_1')
        ax[0].plot(self.controltime, self.matrix_k_2, marker = '*',markersize=2,linewidth=1.0, label='matrix_k_2')
        ax[0].plot(self.controltime, self.matrix_k_3, marker = '*',markersize=2,linewidth=1.0, label='matrix_k_3')
        ax[0].legend(fontsize='medium')
        ax[0].grid(True)
        ax[0].set_title('matrix_k')
        ax[0].set_xlabel('Time-s')
        ax[0].set_ylabel(' ')

        ax[1].plot(self.controltime, self.matrix_q_0, marker = '*',markersize=2,linewidth=1.0, label='matrix_q_0')
        ax[1].plot(self.controltime, self.matrix_q_1, marker = '*',markersize=2,linewidth=1.0, label='matrix_q_1')
        ax[1].plot(self.controltime, self.matrix_q_2, marker = '*',markersize=2,linewidth=1.0, label='matrix_q_2')
        ax[1].plot(self.controltime, self.matrix_q_3, marker = '*',markersize=2,linewidth=1.0, label='matrix_q_3')
        ax[1].legend(fontsize='medium')
        ax[1].grid(True)
        ax[1].set_title('matrix_q')
        ax[1].set_xlabel('Time-s')
        ax[1].set_ylabel(' ')
    ...
    def plot_speed_pid(self):
        ...
        ax[1].plot(self.controltime, self.acceleration_lookup, label='Lookup Acc')
        ax[1].plot(self.controltime, self.acc_close, label='Acc Close')#speed_PID的输出
        ax[1].legend(fontsize='medium')
        ax[1].grid(True)
    ...
    def plot_chassis(self):
        ...
        ax[0].get_shared_x_axes().join(ax[0], ax[1])
        ax[0].plot(
            self.canbustime, self.canbus_throttlefbk,marker = '*',markersize=2,label='Throttle Feedback')
        ax[0].plot(
            self.controltime, self.throttlecmd,marker = '*',markersize=2, label='Throttle Command')
        ax[0].plot(
            self.canbustime, self.canbus_brakefbk, marker = '*',markersize=2,label='Brake Feedback')
        ax[0].plot(
            self.controltime, self.brakecmd, marker = '*',markersize=2,label='Brake Command')
        ax[0].legend(fontsize='medium')
        ax[0].grid(True)
        ...
        ax[1].plot(
            self.canbustime, self.canbus_Driving_mode, label='Driving_mode')
        ax[1].plot(
            self.controltime, self.drivingAction, label='drivingAction')
        ax[1].plot(
            self.controltime, self.path_remain,marker = '*',markersize=2, label='path_remain')
        # ax[1].plot(
        #     self.planningtime, self.numpoints_sum, marker = '*',markersize=2, label='numpoints')
        ax[1].legend(fontsize='medium')
        ax[1].grid(True)
    ...
    def show_longitudinal_all(self):
        ...
        ax[0, 0].plot(self.controltime, self.current_speed,marker = '*',markersize=2, label='current_speed')
        ax[0, 0].plot(self.controltime, self.preview_speed_reference,marker = '*',markersize=2, label='preview_speed_reference')
        ax[0, 0].plot(
            self.controltime, self.speed_reference,marker = '*',markersize=2, linewidth=1.0, label='speed_reference')
        ...
        ax[1, 2].plot(self.controltime, self.is_full_stop,marker = '*',markersize=2, label='is_full_stop')
        # ax[1, 2].plot(
        #     self.controltime, self.speed_integral,marker = '*',markersize=2, label='pid_saturation_status')
        ax[1,2].plot(self.canbustime, self.gear_location, marker = '*',markersize=2,linewidth=1.0, label='gear_location')
        ax[1,2].plot(self.planningtime, self.trajectory_gear, marker = '*',markersize=2,linewidth=1.0, label='trajectory_gear')
        ax[1,2].plot(self.controltime, self.path_remain,marker = '*',markersize=2, label='path_remain')
        ...
        #     self.controltime, self.path_remain, marker = '*',markersize=2, label='path_remain')
        ax[1, 2].plot(
            self.planningtime, self.trajectory_type, marker = '*',markersize=2, label='trajectory_type')
    ...
    def plot_heading(self):
        ...
        ax[2].plot(
            self.canbustime, self.canbus_steerfbk, label='Steering Feedback')
        ax[2].plot(
    ...
    def plot_lateral_all(self):
        ...
        ax[0,0].plot( self.controltime, self.heading_error, label='heading_error')
        ax[0,0].plot( self.controltime, self.lateral_error, label='lateral_error')
        ax[0,0].plot( self.controltime, self.heading_error_feedback, label='lateral_error_feedback')
        ax[0,0].plot( self.controltime, self.lateral_error_feedback, label='lateral_error_feedback')
        ax[0,0].legend(fontsize='medium')
        ax[0,0].grid(True)
        ax[0,0].set_title('Error Info')
        ax[0,0].set_xlabel('Time')

        # ax[0,1].plot( self.controltime, self.heading_error_rate,label='heading_error_rate')
        # ax[0,1].plot(self.controltime,self.lateral_error_rate,label='lateral_error_rate')
        ax[0,1].plot(self.controltime, self.steer_angle_lateral_contribution, label='steer_angle_lateral_contribution')
        ax[0,1].plot(self.controltime, self.steer_angle_lateral_rate_contribution, label='steer_angle_lateral_rate_contribution')
        ax[0,1].plot(self.controltime, self.steer_angle_heading_contribution, label='steer_angle_heading_contribution')
        ax[0,1].plot(self.controltime, self.steer_angle_heading_rate_contribution, label='steer_angle_heading_rate_contribution')
        ax[0,1].legend(fontsize='medium')
        ax[0,1].grid(True)
        ax[0,1].set_title('IMU Info')
        ax[0,1].set_xlabel('Time')

        ax[1,0].plot(self.canbustime, self.canbus_steerfbk, label='Steering Feedback')
        ...
        ax[1,1].plot(self.controltime, self.steer_angle_feedback, label='steer_angle_feedback')
        ax[1,1].plot(self.controltime, self.steer_angle_feedforward, label='steer_angle_feedforward')
        ax[1,1].plot(self.controltime, self.steer_angle_feedback_augment, label='steer_angle_feedback_augment')
        ax[1,1].legend(fontsize='medium')
        ax[1,1].grid(True)
    ...
    def plot_localization(self):
        ...
        self.axarr.plot(
                self.planningx, self.planningy, color='blue', linewidth=2.0, linestyle='--',label='planning path')
        self.axarr.plot(
            self.carx, self.cary, color='red' ,linewidth=1.0, label='pose')
        self.axarr.plot( 
            self.planning_pathx, self.planning_pathy, color='green',linewidth=1.0, label='planning path remain')
    ...
    def add_longitudinal_sheet(self):
        ...
        sheet.write_column(1, 9, self.speed_controller_input_limited, book_format)
    ...
    def add_planning_sheet(self,entity):
        ...
        pathx_sheet.write(self. count, 0,entity.header.timestamp_sec,book_format)
        pathy_sheet.write(self. count, 0,entity.header.timestamp_sec,book_format)
        paths_sheet.write(self. count, 0,entity.header.timestamp_sec,book_format)
        patha_sheet.write(self. count, 0,entity.header.timestamp_sec,book_format)
        pathv_sheet.write(self. count, 0,entity.header.timestamp_sec,book_format)
    ...
    def Print_len(self):
        ...
        print(str(len(self.speed_controller_input_limited))+"-speed_controller_input_limited")
        print(str(len(self.acc_open))+"-acc_open")
        print(str(len(self.acc_close))+"-acc_close")
        print(str(len(self.slope_offset_compensation))+"-slope_offset_compensation")
        ...
        print(str(len(self.station_kp))+"-station_kp")
        print(str(len(self.speed_kp))+"-speed_kp")
        print(str(len(self.path_remain))+"-path_remain")

        # print(str(len(numpoints))+"-numpoints")
    
def is_record_file(path):
        """Naive check if a path is a record."""
        return path.endswith('.record') or fnmatch.fnmatch(path, '*.record.?????') or fnmatch.fnmatch(path, '*.record.????')
        #return path.endswith('.record') or fnmatch.fnmatch(path, '*.record.[0-9][0-9][0-9][0-9][0-9]')
